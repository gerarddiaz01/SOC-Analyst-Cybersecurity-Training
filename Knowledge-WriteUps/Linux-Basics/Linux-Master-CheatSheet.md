...
* **Actions :**
    1.  Utiliser la commande `ls -la` pour repérer les fichiers cachés (commençant par `.`) dans le dossier home [cite: 89-91, 211].
    2.  Créer un dossier `Enquete` **ET** entrer dedans en une seule ligne de commande grâce à l'opérateur `&&` (ex: `mkdir Enquete && cd Enquete`) [cite: 146-149].
    3.  Vérifier la position actuelle avec `pwd` [cite: 98-100].
...
* **Actions :**
    1.  Utiliser `echo` pour écrire "Début de l'incident" dans un fichier `rapport.txt` (Opérateur `>` pour écraser/créer) [cite: 150-155].
    2.  Utiliser `echo` pour **ajouter** "Preuve 1 collectée" à la suite du fichier sans l'écraser (Opérateur `>>`) [cite: 156-159].
    3.  Ouvrir ce fichier avec `nano`, ajouter une ligne manuellement "Analyse terminée", et sauvegarder (`Ctrl+X`, `Y` pour Nano) [cite: 439-450].
    4.  Afficher le résultat final avec `cat` pour vérifier l'intégrité [cite: 110-115].
...
* **Actions :**
    1.  Tenter de créer un fichier dans `/etc` (ex: `touch /etc/test_hack`).  L'action doit échouer (**Permission Denied**) car c'est un dossier système protégé [cite: 402-405].
    2.  Aller dans `/tmp` (dossier temporaire inscriptible par tous) et créer un fichier `malware_sample.txt` [cite: 424-429].
    3.  Changer d'utilisateur avec `su` (ou `sudo su` pour passer root) [cite: 380-383, 778-786].
    4.  Changer le propriétaire du fichier avec `chown` (ex: `chown root:root malware_sample.txt`) et vérifier avec `ls -l` que l'utilisateur standard ne peut plus le modifier [cite: 366-370].
...
* **Actions :**
    1.  Lancer une commande longue en arrière-plan avec l'opérateur `&` (ex: `sleep 500 &`) [cite: 142-145].
    2.  Vérifier qu'elle tourne avec `ps aux` et noter son **PID** (Process ID) [cite: 547-548].
    3.  Arrêter le processus avec `kill [PID]` [cite: 554-555].
...
* **Actions :**
    1.  **Serveur (Machine A) :** Lancer un serveur web Python instantané sur le port 8000 (`python3 -m http.server 8000`) [cite: 517-524].
    2.  **Client (Machine B) :** Télécharger un fichier depuis la Machine A avec `wget http://[IP_A]:8000/fichier` [cite: 473-477, 529].
    3.  **Transfert Sécurisé :** Copier un fichier sensible de A vers B via SSH avec `scp` (ex: `scp secret.txt user@IP_B:/tmp/`) [cite: 479-487, 495-504].
