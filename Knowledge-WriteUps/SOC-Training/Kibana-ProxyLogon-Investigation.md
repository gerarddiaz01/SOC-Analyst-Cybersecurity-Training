...
Furthermore, while containing the compromised accounts addresses the symptoms, the root cause of the intrusion must be remediated immediately. The Exchange Server was compromised via CVE-2021-26855. Until the emergency out-of-band security patches provided by Microsoft for ProxyLogon are applied, the server remains exposed to unauthenticated remote code execution. Without this critical patching, any evicted attacker can simply re-exploit the `/ecp/proxyLogon.ecp` endpoint to regain access within minutes.
